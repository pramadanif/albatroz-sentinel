...
import { useContractData, useContractEvents, SENTINEL_ADDRESS, VAULT_ADDRESS } from '../../hooks/useContractData';
...
const TerminalLog: React.FC = () => {
  ...
  useEffect(() => {
    ...
    setLogs([
      ...
      { id: 3, timestamp: new Date().toLocaleTimeString(), message: `SENTINEL_STATUS: ACTIVE (${SENTINEL_ADDRESS.slice(0, 8)}...${SENTINEL_ADDRESS.slice(-4)})`, type: 'success' },
      { id: 4, timestamp: new Date().toLocaleTimeString(), message: `VAULT_PROXY: CONNECTED (${VAULT_ADDRESS.slice(0, 8)}...${VAULT_ADDRESS.slice(-4)})`, type: 'success' },
    ]);
  }, []);
  ...
  const handleEvent = React.useCallback((type: 'RATE_UPDATE' | 'STRATEGY_EXECUTION' | 'DEPOSIT' | 'WITHDRAW' | 'REACTIVE_CALLBACK', data: any) => {
    // Determine if we should process this event (filtering logic)
    // We remove the mounted check here since proper cleanup is handled by hook
    const now = new Date();
    ...
    if (type === 'RATE_UPDATE') {
      ...
      const targetAddr = data.target && typeof data.target === 'string' ? data.target.slice(0, 6) : 'unknown';
      message = `→ REACTIVE_NET: CALLBACK SENT | TARGET: ${targetAddr}...`;
      logType = 'decision';
      txHash = data.txHash;
    }
        ...
        {logs.map((log, idx) => (
          ...
          >
            ...
            <span className={`${getLogColor(log.type)} flex-1 break-words font-mono`}>
              ...
              {log.txHash && (
                <a
                  href={log.message.includes('REACTIVE_NET')
                    ? `https://lasna.reactscan.net/tx/${log.txHash}`
      ...
      <div className="border-t border-[#333333] bg-[#111111] p-4">
        ...
        <div className="flex gap-3">
          <button
            ...
            className={`flex-1 py-2 px-3 text-xs font-mono uppercase font-bold border transition-all ${strategy === 'conservative'
          ...
          <button
            ...
            className={`flex-1 py-2 px-3 text-xs font-mono uppercase font-bold border transition-all ${strategy === 'aggressive'
